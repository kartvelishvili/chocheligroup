...
import { newsCategoriesApi, newsApi } from '@/lib/apiClient';

export const seedDemoNews = async () => {
  try {
    // 1. Ensure we have at least one category
    const categories = await newsCategoriesApi.getAll();

    let categoryId;
    if (!categories || categories.length === 0) {
      const newCat = await newsCategoriesApi.create({
        name_ka: 'ზოგადი',
        name_en: 'General',
        slug: 'general',
        is_active: true,
    ...
    // 3. Upsert articles (check existence by slug first)
    for (const article of demoArticles) {
      const allNews = await newsApi.getAll();
      const existing = allNews.find(n => n.slug === article.slug);

      if (!existing) {
        await newsApi.create(article);
      }
    }
