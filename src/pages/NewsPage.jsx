...
import { newsApi, newsCategoriesApi } from '@/lib/apiClient';
...
const NewsPage = () => {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const cats = await newsCategoriesApi.getActive();
      setCategories(cats || []);

      const newsData = await newsApi.getPublished();
      setNews(newsData || []);
    } catch (err) {
  ...
  return (
    <>
      ...
      <div style={{ backgroundColor: d.bg, minHeight: '100vh', paddingBottom: '5rem' }}>
        {/* Hero */}
        <div className="py-28 mb-12 relative overflow-hidden" style={{ background: `linear-gradient(135deg, ${d.heroBg || d.bg}, ${(d.heroBg || d.bg)}DD)` }}>
          ...
          <div className="container mx-auto px-6 text-center relative z-10">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="inline-block px-5 py-2 rounded-full text-sm font-semibold mb-6" style={{ backgroundColor: `${d.accent}15`, color: d.heroText || '#fff', border: `1px solid ${d.accent}30` }}>
              {t({ en: 'Latest Updates', ka: 'უახლესი სიახლეები' })}
            </motion.div>
            ...
            <p className="max-w-2xl mx-auto text-lg body-font" style={{ color: (d.heroText || '#fff') + 'AA' }}>
