...
import { siteSettingsApi, newsApi, newsCategoriesApi, menuItemsApi, companiesApi } from '@/lib/apiClient';
import { 
  Newspaper, 
  FolderTree, 
  Menu as MenuIcon, 
...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchCacheVersion = async () => {
      try {
        ...
        const cv = data.find(s => s.key === 'cache_version');
        if (cv) setCurrentCacheVersion(cv.value);
      } catch (e) {
  ...
  const handleClearCache = async () => {
    ...
    try {
      ...
      await siteSettingsApi.upsert('cache_version', newVersion);
      ...
      toast({
        title: '❌ შეცდომა',
        description: `ქეშის გასუფთავება ვერ მოხერხდა: ${error.message}.`,
        variant: 'destructive',
      });
    } finally {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      setLoading(true);
      try {
        const [newsData, categories, menuItems, companies] = await Promise.all([
          newsApi.getAll(),
          newsCategoriesApi.getAll(),
          menuItemsApi.getAll(),
          companiesApi.getAll()
        ]);
        ...
        const news = (newsData || []).sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at));
        setStats({
