...
/* ══════════ FOUNDER SECTION ══════════ */
const FounderSection = ({ d, lang }) => {
  const { content: founder } = useSiteContent('founder_highlight');
      ...
      <div className="container mx-auto px-6 max-w-6xl">
        <div className="text-center mb-12">
          <span className="font-bold tracking-widest uppercase text-xs mb-3 block" style={{ color: d.accent }}>
            {lang === 'ka' ? 'დამფუძნებელი' : 'Founder'}
          </span>
          <h2 className="text-3xl md:text-4xl font-bold heading-font" style={{ color: d.heading }}>
            {founder[`name_${lang}`] || (lang === 'ka' ? 'ცეზარ ჩოჩელი' : 'Tsezar Chocheli')}
          </h2>
        </div>
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="rounded-2xl overflow-hidden shadow-xl">
            <img src={founder.image_url || "https://s3.ihost.ge/site-chocheligroup-com/images/506854830-10013410878742299-1013126184738099367-n.jpg"} alt={founder[`name_${lang}`]} className="w-full h-auto object-cover" />
          </motion.div>
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="space-y-6">
            <p className="text-lg leading-relaxed body-font" style={{ color: d.text }}>
              {founder[`bio_paragraph1_${lang}`] || ''}
            </p>
            <p className="text-lg leading-relaxed body-font" style={{ color: d.textMuted }}>
              {founder[`bio_paragraph2_${lang}`] || ''}
            </p>
            <Link to={lang === 'ka' ? '/founder' : '/founder-en'} className="inline-flex items-center gap-2 px-6 py-3 rounded-full font-bold text-sm heading-font text-white shadow-lg transition-all hover:shadow-xl" style={{ backgroundColor: d.accent }}>
              {lang === 'ka' ? 'სრული ბიოგრაფია' : "Founder's Full Story"} <ArrowRight className="w-4 h-4" />
            </Link>
          </motion.div>
        </div>
      </div>
