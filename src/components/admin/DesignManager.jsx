...
import { siteDesignApi } from '@/lib/apiClient';
...
const DesignManager = () => {
  ...
  const fetchPresets = useCallback(async () => {
    setLoading(true);
    const data = await siteDesignApi.getAll();
    setPresets(data || []);
    setLoading(false);
  }, []);
  ...
  const activatePreset = async (sectionKey, presetNumber) => {
    ...
    try {
      await siteDesignApi.activate(sectionKey, presetNumber);
  ...
  const applyThemeToAll = async (presetNumber) => {
    ...
    try {
      ...
      for (const sectionKey of sections) {
        ...
        if (target) {
          await siteDesignApi.activate(sectionKey, target.preset_number);
        }
      }
  ...
  const changeLayout = async (sectionKey, layoutValue) => {
    ...
    try {
      ...
      if (matchingPreset && !matchingPreset.is_active) {
        ...
        await siteDesignApi.update(activePreset.id, { layout: layoutValue });
