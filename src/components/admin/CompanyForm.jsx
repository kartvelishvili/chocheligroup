...
const CompanyForm = ({ company = null, initialSubProjects = [], onSuccess, onCancel }) => {
  ...
  const handleImageUpload = async (e) => {
    ...
    setUploading(true);
    try {
      const result = await uploadApi.upload('site-assets', file);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (companyId) {
        ...
        const newCompany = await companiesApi.create(payload);
        companyId = newCompany.id;
      }

      if (companyId) {
         if (deletedSubProjectIds.length > 0) {
            await subBrandsApi.deleteMany(deletedSubProjectIds);
         }
         for (let i = 0; i < subProjects.length; i++) {
            ...
            if (sp.id) {
               await subBrandsApi.update(sp.id, spPayload);
            } else {
               await subBrandsApi.create(spPayload);
            }
         }
      }
